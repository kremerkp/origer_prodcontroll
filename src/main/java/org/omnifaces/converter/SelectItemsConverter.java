 ...
 * <p>
 * This class can be used as-is provided the Object has a <code>toString</code> method that uniquely identifies it and which makes sense as an identifier. Since this is not the typical case for <code>toString</code>, users are assumed to override the <code>getAsString</code> method and provide an implementation that converts an object to its string representation. This <code>getAsObject</code> method will then be used to automatically do the reverse mapping.
 * <p>
 ...
 * </pre>
 * <p>
 * When extending this converter to override <code>getAsString()</code> to return something simpler, such as the entity ID by <code>return String.valueOf(((Entity) value).getId());</code>, you only have to change the <code>converter</code> attribute in the above example to refer the extended converter instead.
 *
 * @author Arjan Tijms
 *
 */
